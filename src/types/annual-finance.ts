...
export interface AnnualInput {
  ...
  fxDate: string;
  transaction: Value;
  acquisitionBundled: boolean;
  area: Value;
  renovation: Value;
  capacity: Value;
  ffe: Value;
  medical: Value;
  equipmentBundled: boolean;
  hydro: Value;
  soft: Value;
  contingency: Value;
  ...
  preOpeningBundled: boolean;
  phases: { year: number; capacity: Value }[];
  spending: number[];
  services: AnnualService[];
  ancillary: Value;
  costMode: "derived" | "margin";
  margin: Value;
  fixed: Value;
  maintenance: Value;
  provenance: string;
