...
import { identities } from '../src/data/property-identities.ts';
const origin=process.argv[2]??'http://127.0.0.1:4173';
const data=JSON.parse(fs.readFileSync('src/data/research.json','utf8'));
const failures=[];let routes=0;
for(const locale of ['de','en','ka'])for(const suffix of ['', '/sanatoriums','/news','/project','/sources','/scenarios','/compare','/finance','/evidence','/report','/methodology',...identities.map(a=>'/sanatoriums/'+a[0])]){const r=await fetch(`${origin}/${locale}${suffix}/`);const body=await r.text();routes++;if(r.status!==200||!body.includes('Tskaltubo'))failures.push(`${locale}${suffix}: ${r.status}`);}
